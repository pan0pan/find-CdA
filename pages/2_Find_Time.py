import streamlit as st
import time
import numpy as np
from findCd import getCdA, getTime
#  getTime(g,p,c,Height, mass_kg, Cd,Area):
# getCdA(g,p,c,CdA_init, TimeTest, Height, mass_kg, error_rate=0.01, max_iter=50, cdA_max_limit=100.0):


st.set_page_config(page_title="Find Time", page_icon="📈")

st.markdown("# โปรแกรมหาค่าระยะเวลาในการตก")
st.write(
    """โปรแกรมนี้เป็นโปรแกรมที่จัดทำขึ้นเพื่อ ประยุกต์ใช้คณิตศาตร์ในการหาค่าระยะเวลาในการตก
    โดยคิดแรงต้านอากาศร่วมด้วย"""
)

#side bar

st.sidebar.header("ตั่งค่าเชิงลึก setting")
g = st.sidebar.number_input("แรงโน้มถ่วง (m/s²)",value=9.81)
p = st.sidebar.number_input("แรงดันอากาศโดยเฉลี่ย ณ พื้นที่นั้น kg/m³" , value= 1.2)
c = st.sidebar.number_input("ช่องว่างเวลาในการคำนวณ (s)", value = 0.001)



mass = st.number_input("มวลของวัตถุ(g)")
height = st.number_input("ความสูง(m)")
Cd = st.number_input("ค่าสัมประสิทธิ์แรงต้านอากาศ")
Area = st.number_input("พื้นที่หน้าตัดของวัตถุ (m)")


if st.button("เริ่มการคำนวณ"):
    st.header("ผลการหาค่า CdA")
   
    if height <=0 or Cd <= 0 or mass <= 0 or Area <=0:
        
        st.warning(f"ไม่สามารถคำนวณได้โปรตัวสอบว่า มีค่าไดเป็น 0 หรือไม่")
    else :    
        try :
            time_output, acc_plot, vel_plot = getTime(g,p,c,height, mass* 1000, Cd,Area)
        except:
            st.warning(f"ไม่สามารถคำนวณได้โปรตัวสอบว่าใส่ค่าถูกต้องหรือไม่")
        else:
            st.write(f"ระยะเวลาที่คำนวณได้คือ {time_output}")
            st.line_chart(acc_plot, x_label="ความเร่ง(m/s²)", y_label= "เวลา(s)")
            st.line_chart(vel_plot, x_label="ความเร้ว(m/s)", y_label= "เวลา(s)")


